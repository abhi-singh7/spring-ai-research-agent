...
public class McpToolRouter {
    ...
    public static final List<String> DEFAULT_BACKEND_CHAIN = List.of("firecrawl", "ddg", "ollama_web_search", "tavily");

    private final Map<String, List<String>> routingMap = new LinkedHashMap<>();

    public McpToolRouter() {

        // firecrawl (self-hosted Firecrawl API, POST /v2/search) is the preferred engine; duckduckgo is the
        // independent fallback that keeps working even when Firecrawl is down or returns no results;
